...
export class RankList extends Component {
  ...
  private async Init() {
    this.RegCallBack();
    this.rankFarmePre = (await BundleManager.Instance.loadAssetsFromBundle(
      "PartPrefabs",
      "RankFarme",
    )) as Prefab;
    this.roleFarmePre = (await BundleManager.Instance.loadAssetsFromBundle(
      "PartPrefabs",
      "RoleFarme",
    )) as Prefab;
  }
  ...
  private OnDragEvent() {
    ...
    if (
      this.pointer + 24 < 100 &&
      this.listcontent.position.y >= this.listStartY + 16 * 120
    ) {
      if (
        this.board
          .getChildByPath("RankView")
      ...
      //此处更新物体信息
      //.....
      this.listcontent.setPosition(
        new Vec3(0, this.listcontent.position.y - downLoad * 120, 0),
      );
      return;
    }
    ...
    if (this.pointer > 0 && this.listcontent.position.y <= this.listStartY) {
      ...
      //.....
      this.listcontent.setPosition(
        new Vec3(0, this.listcontent.position.y + upLoad * 120, 0),
      );
    }
  }
  ...
  private async ShowUserData(_userData: UserData, _avatar: Node) {
    try {
      ...
      this.ShowAvatar(
        ...
        _avatar.getChildByPath("Mask/Sprite"),
      );
      //显示用户信息
      //...
    } catch (error) {
  ...
  private ShowRankList(_rank_Info: Map<rank_item, UserRankInfo>) {
    ...
    for (let [key, value] of _rank_Info) {
      ...
      if (key.rank <= 3) {
        switch (key.rank) {
          case 1:
            t.getChildByPath("RankFlag").children[0].active = true;
            break;
          case 2:
            t.getChildByPath("RankFlag").children[1].active = true;
            break;
          case 3:
            t.getChildByPath("RankFlag").children[2].active = true;
            break;
        }
      }
      t.getChildByPath("RankNmber").getComponent(Label).string = "" + key.rank;
      t.getChildByPath("UserName").getComponent(Label).string =
        "" + value.nick_name;
  ...
  private async ShowRoleList(_roleList: Role[], _parent: Node) {
    for (let i = 0; i < _roleList.length; i++) {
      ...
      t.getChildByPath("Mask/Sprite").getComponent(Sprite).spriteFrame =
        await loadAssets.LoadImg(str);
      switch (_roleList[i].Level) {
        case 1:
          t.getChildByPath("Farme")
            .getComponent(Sprite)
