...
export class netPlayer {
  ...
  public cb_achievement_complete: (
    achieve: common.UserAchievement,
    wAchieve: common.UserWeekAchievement,
  ) => void;
  public cb_rank_reward: (
    reward: common.RankReward,
  ...
  public constructor() {
    ...
    this.c_player_login_caller = new player_login.player_login_caller(
      cli.cli_handle,
    );
    this.c_rank_cli_service_caller = new rank_cli.rank_cli_service_caller(
      cli.cli_handle,
    );

    this.player_client_module = new player_client.player_client_module(
      cli.cli_handle,
    );
    this.c_player_shop_caller = new player_login.player_shop_caller(
      cli.cli_handle,
    );
    ...
    this.player_client_module.cb_achievement_complete = (
      achieve: common.UserAchievement,
      wAchieve: common.UserWeekAchievement,
    ) => {
  ...
  private login_callback(
    ...
    console.log("token:" + token + " is_new:" + is_new);
    if (!is_new) {
      this.c_player_login_caller
        .get_hub(this.player_name)
        .player_login(token, nick_name, avatar_url)
        .callBack(
          (info) => {
            this.UserData = info;
      ...
      this.cb_player_login_non_account.call(null, token);
    }
  }
    ...
    cli.cli_handle.get_hub_info("login", (login_hub) => {
      if (login_hub) {
        if (login_type == "no_author") {
          this.c_login_caller
            .get_hub(login_hub.hub_name)
            .player_login_no_token(code)
            .callBack(
              (player_name, token, is_new) => {
                this.login_callback(
                  is_new,
                  player_name,
                  token,
  ...
  public create_role(
    ...
    this.c_player_login_caller
      .get_hub(this.player_name)
      .create_role(code, name, nick_name, avatar_url)
      .callBack(
        (info) => {
          this.UserData = info;
          this.cb_player_login_sucess.call(null);
  ...
  //购买卡包
  public cb_buy_card_packet: (
    _cardPacketInfo: player_login.CardPacket,
    _bagInfo: common.Bag,
  ) => void;
  ...
  public edit_role_group(_roleGroup: common.RoleGroup) {
    this.c_player_shop_caller
      .get_hub(this.player_name)
      .edit_role_group(_roleGroup)
      .callBack(
        (info: common.UserData) => {
  ...
  //获取个人排名
  public cb_get_rank_guid: (
    _rank: rank_item,
    _Info: common.UserRankInfo,
  ) => void;
  public get_rank_guid(_rank_name: string, _guid: number) {
    this.c_rank_cli_service_caller
      .get_hub(this.rank_name)
      .get_rank_guid(_rank_name, _guid)
      .callBack(
        (_rank: rank_item) => {
          if (_rank != null && _rank.item != null) {
  ...
  //获取排行榜区间
  public cb_get_rank_range: (
    _rank_Info: Map<rank_item, common.UserRankInfo>,
  ) => void;
  public get_rank_range(_rank_name: string, _start: number, _end: number) {
    this.c_rank_cli_service_caller
      .get_hub(this.rank_name)
      .get_rank_range(_rank_name, _start, _end)
      .callBack(
        (_rank_list: rank_item[]) => {
          let rankinfo = new Map<rank_item, common.UserRankInfo>();

          for (let i = 0; i < _rank_list.length; i++) {
            if (_rank_list[i] != null && _rank_list[i].item != null) {
              let t = common.protcol_to_UserRankInfo(
                decode(_rank_list[i].item),
              );
