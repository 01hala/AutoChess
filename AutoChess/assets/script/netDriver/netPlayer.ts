...
export class netPlayer {
    ...
    private login_callback(is_new:boolean, player_name:string, token:string, nick_name:string, avatar_url:string) {
        ...
        cli.cli_handle.get_hub_info("login", (login_hub)=>{
            if(login_hub) {
                if (login_type == "no_author") {
                    this.c_login_caller.get_hub(login_hub.hub_name).player_login_no_token(code).callBack((player_name, token, is_new)=>{
                        this.login_callback(is_new, player_name, token, nick_name, avatar_url);
                    }, (err)=>{
                    ...
                    this.c_login_caller.get_hub(login_hub.hub_name).player_login_wx(code).callBack((player_name, token, is_new)=>{
                        this.login_callback(is_new, player_name, token, nick_name, avatar_url);
                    }, (err)=>{
                        console.log("login error:" + err);
                    }).timeout(20000, ()=>{
                    ...
                    this.c_login_caller.get_hub(login_hub.hub_name).player_login_dy(code).callBack((player_name, token, is_new)=>{
                        this.login_callback(is_new, player_name, token, nick_name, avatar_url);
                    }, (err)=>{
                        console.log("login error:" + err);
                    }).timeout(20000, ()=>{
    ...
    public create_role(code:string, name:string, nick_name:string, avatar_url:string){
        console.log("begin create role!");
        this.c_player_login_caller.get_hub(this.player_name).create_role(code, name, nick_name, avatar_url).callBack((info)=>{
            this.UserData = info;
            this.cb_player_login_sucess.call(null);
        }, (err)=>{
